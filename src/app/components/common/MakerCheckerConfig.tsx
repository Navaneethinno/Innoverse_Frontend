...
import type { CheckerAssignmentIn } from "../../features/maker-checker.types";

export type CheckerMode = "ANY" | "ASSIGNED_PARALLEL" | "ASSIGNED_SEQUENTIAL";

export interface CheckerCandidate {
  id: string | number;
  name: string;
  institution_id?: string | number | null;
  is_current_maker?: boolean;
  is_active?: boolean;
}

export interface MakerCheckerConfigValue {
...
interface Props {
  value: MakerCheckerConfigValue;
  onChange: (value: MakerCheckerConfigValue) => void;
...
function nextSequence(assignments: CheckerAssignmentIn[]) {
...
export function MakerCheckerConfig({
  ...
  const eligibleCandidates = useMemo(
    () =>
      candidates.filter((candidate) => {
        if (candidate.is_current_maker || String(candidate.id) === String(currentMakerId)) return false;
        if (makerInstitutionId !== undefined && makerInstitutionId !== null && candidate.institution_id !== undefined && candidate.institution_id !== null) {
  ...
  const selectedCandidates = value.checker_assignments
    .map((assignment) => eligibleCandidates.find((candidate) => String(candidate.id) === String(assignment.user_id)))
    .filter(Boolean) as CheckerCandidate[];

  const setMode = (checker_mode: CheckerMode) => {
    if (checker_mode === "ANY") {
      onChange({ checker_mode, checker_assignments: [], required_checker_count: Math.max(1, value.required_checker_count || 1) });
      return;
    }
    ...
    onChange({
      ...
      required_checker_count: checker_mode === "ASSIGNED_SEQUENTIAL" ? count : Math.min(Math.max(1, value.required_checker_count || count), count),
    });
  };

  const addCandidate = (id: string | number) => {
    if (selectedIds.includes(String(id))) return;
    const candidate = eligibleCandidates.find((item) => String(item.id) === String(id));
    if (!candidate) return;
    const assignment: CheckerAssignmentIn = {
      user_id: candidate.id,
      sequence: value.checker_mode === "ASSIGNED_SEQUENTIAL" ? nextSequence(value.checker_assignments) : undefined,
    };
  ...
  const removeCandidate = (id: string | number) => {
    const nextAssignments = value.checker_assignments.filter((assignment) => String(assignment.user_id) !== String(id));
    ...
    onChange({
      ...
      required_checker_count:
        value.checker_mode === "ASSIGNED_SEQUENTIAL"
          ...
          : Math.min(Math.max(1, value.required_checker_count || 1), Math.max(1, normalizedAssignments.length)),
    });
  };

  const moveAssignment = (fromIndex: number, toIndex: number) => {
    ...
    onChange({
      ...value,
      checker_assignments: nextAssignments.map((assignment, index) => ({ ...assignment, sequence: index + 1 })),
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white/90 p-4 space-y-4">
      ...
      <div className="space-y-2">
        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Approval Method</p>
        <div className="grid gap-2 sm:grid-cols-3">
          {[
            ...
            <button
              key={option.value}
              type="button"
              onClick={() => setMode(option.value as CheckerMode)}
              className={cn(
      ...
      {value.checker_mode !== "ANY" && (
        <div className="space-y-3">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2">Assigned Checkers</p>
            <div className="flex flex-wrap gap-2">
              {eligibleCandidates.length === 0 ? (
                <span className="text-xs text-slate-400">No eligible checker candidates available</span>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  onChange({ ...value, required_checker_count: value.checker_mode === "ASSIGNED_SEQUENTIAL" ? value.checker_assignments.length : next });
                }}
              ...
              <div>
                ...
                <div className="rounded-xl border border-slate-100 bg-slate-50 p-3 space-y-2">
                  {selectedCandidates.length === 0 ? (
                    <p className="text-xs text-slate-400">Select checkers to define approval order</p>

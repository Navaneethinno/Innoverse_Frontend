...
export const institutionLegalApi = { add: (p) => request(ENDPOINTS.ADD, p), submit: (p) => request(ENDPOINTS.SUBMIT, p), edit: (p) => request(ENDPOINTS.EDIT, p), auth: (p) => request(ENDPOINTS.AUTH, p), deauth: (p) => request(ENDPOINTS.DEAUTH, p), delete: (p) => request(ENDPOINTS.DELETE, p), deleteAuth: (p) => request(ENDPOINTS.DELETE_AUTH, p), deactivate: (p) => request(ENDPOINTS.DEACTIVATE, p), reactivate: (p) => request(ENDPOINTS.REACTIVATE, p), list: (p = { page: 1, limit: 10 }) => request(ENDPOINTS.LIST, p), getActive: (p = { view: "dropdown" }) => request(ENDPOINTS.GET_ACTIVE, p), audit: (p) => request(ENDPOINTS.AUDIT, p), pending: (p) => request(ENDPOINTS.PENDING, p) };

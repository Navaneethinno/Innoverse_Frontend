  ...
  return (
    <div className="relative px-2" ref={containerRef}>
      <button
        ...
        className={cn(
          "flex items-center gap-2.5 rounded-xl h-10 text-xs font-bold text-white transition-colors bg-[var(--primary)] shadow-md shadow-blue-200/50",
      ...
      >
        <span className="flex items-center gap-2 truncate">
          ...
          {!isCollapsed && <span className="truncate">{t("selectModule")}</span>}
        </span>
      ...
      {isOpen && (
        ...
        >
          ...
          {(modules || []).length === 0 && !isCollapsed && (
            <p className="px-3.5 py-2 text-[11px] text-muted-foreground">{t("noModulesAvailable")}</p>
          )}
        </div>
      )}
    </div>

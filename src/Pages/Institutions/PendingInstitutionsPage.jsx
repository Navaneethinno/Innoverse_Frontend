...
export function PendingInstitutionsPage() {
  ...
  const pendingQuery = usePendingInstitutionsQuery(isPlatformOwner);
  const approvalMutation = useInstitutionApprovalMutation();
  const pendingInstitutions = pendingQuery.data ?? [];
  const isLoading = pendingQuery.isLoading;
  const error = pendingQuery.error?.message;
      ...
      >
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1.5 font-medium">
            {isLoading
              ? "Loading…"
              : `${pendingInstitutions.length} pending approval${pendingInstitutions.length !== 1 ? "s" : ""}`}
          </p>
        </div>
        <button

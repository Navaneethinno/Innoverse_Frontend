...
// The signed-in user's own full record (/config/user/get), for the header's
// "My profile" page. The session user from login has the account fields but
// not the KYC ones (email, mobile, address, employee id...).
export function useMyProfileQuery(userId) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await usersApi.get({ user_id: userId });

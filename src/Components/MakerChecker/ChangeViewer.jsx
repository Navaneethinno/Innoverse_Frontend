...
function renderValue(
  value,
  depth = 0,
  beforeValue, // only provided for EDIT nested comparison
  isEditMode = false,
) {
  // Unwrap JSON strings
  if (typeof value === "string") {
    ...
    if (parsed !== null) {
      const parsedBefore =
        typeof beforeValue === "string" ? tryParseJson(beforeValue) : beforeValue;
    ...
    return (
      <div className={cn("space-y-1.5", depth > 0 && "mt-1")}>
        {value.map((item, idx) => (
          <div
            key={idx}
            className="rounded-lg border border-slate-100 bg-slate-50/60 overflow-hidden"
          >
...
function FieldRow({ label, beforeNode, afterNode, changed, isComplex }) {
        ...
        <div className="grid grid-cols-2 gap-2">
          <div
            className={cn(
              ...
              changed ? "border-red-100 bg-red-50/40" : "border-slate-100 bg-slate-50/40",
            )}
          >
          ...
          <div
            className={cn(
              "rounded-lg p-1.5 border",
              changed ? "border-emerald-100 bg-emerald-50/40" : "border-slate-100 bg-slate-50/40",
            )}
          >
  ...
  return (
    <div
      className={cn(
        "grid gap-2 py-2 border-b border-slate-50 last:border-0",
        hasBeforeCol ? "grid-cols-[1fr_1fr_1fr]" : "grid-cols-[1fr_2fr]",
      )}
    >
      ...
      {hasBeforeCol && (
        <span
          className={cn(
            ...
            changed ? "bg-red-50 text-red-700 line-through" : "text-slate-500",
          )}
        >
          {beforeNode}
        </span>
      ...
      <span
        className={cn(
          "text-xs px-2 py-1 rounded-lg",
          changed ? "bg-emerald-50 text-emerald-700 font-semibold" : "text-slate-700",
        )}
      >
        {afterNode}
      </span>
...
function isComplexValue(v) {
  ...
  if (Array.isArray(v))
    return v.length > 0 && v.some((item) => typeof item === "object" && item !== null);
  const obj = v;
  if ("id" in obj && "name" in obj && Object.keys(obj).length <= 3) return false;
  return true;
}
...
// ── Main component ────────────────────────────────────────────────────────────
export function ChangeViewer({ action, before_data, after_data }) {
  ...
  const changedKeys = isEdit
    ? allKeys.filter(
        (k) => !isSensitiveKey(k) && serializeForDiff(before?.[k]) !== serializeForDiff(data?.[k]),
      )
    : [];

...
function checkerWorkflowLabel(req) {
...
function Row({ label, value }) {
  return (
    <div className="flex items-start gap-2">
      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest w-20 shrink-0 pt-0.5">
        {label}
      </span>
...
function ConfirmDialog({ req, decision, onConfirm, onCancel }) {
    ...
    >
      ...
      >
        ...
        <div className="space-y-2 mb-4 rounded-xl bg-slate-50/80 border border-slate-100 p-3">
          <Row
            label="Entity"
            value={`${entityName} · ${ENTITY_DISPLAY[req.entity_type] ?? req.entity_type}`}
          />
          <Row label="Action" value={req.action} />
          <Row label="Maker" value={req.maker?.name ?? "—"} />
          <Row
            label="Progress"
            value={`${req.approval_count} / ${req.required_checker_count} approvals`}
          />
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button
            ...
            className={cn(
              "px-4 py-2 rounded-xl text-xs font-bold text-white shadow-md transition-colors",
              isApprove ? "shadow-emerald-200/50" : "shadow-red-200/50",
            )}
            style={{
              background: isApprove
                ? "linear-gradient(135deg,#6EDFC4,#3BBFA0)"
                : "linear-gradient(135deg,#FF8C6B,#FF5C5C)",
            }}
          >
...
function ExpandedDetail({ req }) {
  const assignments = req.checker_assignments ?? [];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-4">
      ...
      <div className="rounded-xl border border-slate-100 bg-white/60 p-4 space-y-2">
        ...
        ].map(([label, value, isUuid]) => (
          <div key={label} className="flex items-start gap-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest w-24 shrink-0 pt-0.5">
              {label}
            </span>
            <span
              className={cn(
                "break-all flex items-center gap-0.5",
                isUuid
          ...
          <div className="pt-1">
            ...
            <div className="space-y-1">
              {assignments
                ...
                .map((a, idx) => {
                  const name = String(
                    a.checker_name ?? a.user_name ?? a.username ?? a.checker_id ?? a.user_id ?? "—",
                  );
...
export function PendingTable({
  ...
  // Support legacy onApprove/onReject props by wrapping them into onDecision
  const handleDecision =
    onDecision ??
    (async (request_id, decision) => {
      if (decision === "approve" && onApprove) await onApprove(request_id);
      if (decision === "reject" && onReject) await onReject(request_id);
    });
  ...
  const handleConfirm = async (remark) => {
    ...
    await handleDecision(req.request_id, decision, remark);
    setProcessing(null);
  };
  const COLS = [
    "Request / Entity",
              ...
              {COLS.map((h, idx) => (
                <th
                  ...
                  className={cn(
                    ...
                    idx === 0 ? "text-left first:pl-5" : "text-center",
                  )}
                >
                  {h}
                </th>
                ...
                return (
                  <Fragment key={req.request_id}>
                    <motion.tr
                      ...
                      className={cn(
                        "border-b border-slate-50 hover:bg-white/60 transition-colors cursor-pointer",
                    ...
                    >
                      ...
                      <td className="px-4 py-3 pl-5 min-w-[200px] w-[22%]">
                        ...
                        <p className="text-[10px] text-slate-400 mt-0.5">
                          {ENTITY_DISPLAY[req.entity_type] ?? req.entity_type}
                        </p>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={cn(
                            "text-xs font-bold",
                            isFinalized ? "text-emerald-600" : "text-amber-600",
                          )}
                        >
                      ...
                      {/* Remark */}
                      <td className="px-4 py-3 text-xs text-slate-400 max-w-[100px] truncate text-center">
                        {req.remark ?? "—"}
                      </td>
